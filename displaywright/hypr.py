...
One reader, one writer, one event loop, shared by both halves of the app. The
wallpaper side wants the lit outputs and their logical geometry; the arrangement
side wants every output including the disabled ones, plus the ability to push a
layout back. :func:`read_monitors` returns the superset, sorted so the lit ones
come first in left-to-right order -- filtering it is the whole of what the
wallpaper side has to do.
"""
