...
BEGIN = "-- >>> hyprlayout managed block: edited by `hyprlayout`, safe to move as a whole >>>"
END = "-- <<< hyprlayout managed block <<<"

_HEADER = (
    "-- Generated from the hyprlayout GUI. Anything outside this block is left\n"
...
def render_call(state: MonitorState) -> str:
    ...
    The same text is what :func:`hyprlayout.hypr.apply_states` evaluates live, so
    what you preview is exactly what runs.
    """
...
def _comment_out_monitor_calls(text: str, managed: set[str] | None) -> tuple[str, int]:
    ...
    out = [
        ("-- [hyprlayout] replaced: " + line) if index in doomed else line
        for index, line in enumerate(lines)
    ]
    return "\n".join(out) + ("\n" if text.endswith("\n") else ""), commented
...
def merge(existing: str, block: str, managed: set[str] | None = None) -> str:
    ...
    if commented:
        note = (
            f"-- hyprlayout commented out {commented} earlier hl.monitor call(s);\n"
