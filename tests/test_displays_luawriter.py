...
class MergeTests(unittest.TestCase):
    def test_first_save_preserves_user_lines_and_comments_out_monitors(self):
        ...
        self.assertIn(
            '-- [displaywright] replaced: hl.monitor({ output = "eDP-1"', merged
        )
    ...
    def test_multiline_calls_are_fully_commented(self):
        ...
        self.assertIn('-- [displaywright] replaced:   mode = "preferred",', merged)
        self.assertIn('-- [displaywright] replaced: })', merged)
...
class LegacyMarkerTests(unittest.TestCase):
    """A monitors.lua written by hyprlayout is rewritten, not appended to.

    The managed block changed name when the two tools became one. Failing to
    recognise the old markers would leave the old block in place and add a
    second one under it, and Hyprland would apply whichever came last.
    """

    LEGACY_BEGIN = luawriter.LEGACY_MARKERS[0][0]
    LEGACY_END = luawriter.LEGACY_MARKERS[0][1]
    ...
    def test_the_old_block_is_replaced_in_place(self):
        merged = luawriter.merge(self.existing(), luawriter.render_block([laptop()]))
        self.assertNotIn(self.LEGACY_BEGIN, merged)
        self.assertEqual(merged.count(luawriter.BEGIN), 1)
        self.assertIn("hl.env('KEEP', '1')", merged)
        self.assertIn("-- trailing user line", merged)

    def test_nothing_outside_the_old_block_is_commented_out(self):
        merged = luawriter.merge(self.existing(), luawriter.render_block([laptop()]))
        self.assertNotIn("replaced:", merged)
